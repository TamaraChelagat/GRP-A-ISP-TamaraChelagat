...
BATCH_SIZE = 10  # Generate 10 transactions per batch
...
class SyntheticTransactionGenerator:
    ...
    def generate_fraud_transaction(self) -> Dict:
        ...
        # Time (frauds happen at various times)
        time_val = max(0, np.random.normal(
            self.fraud_params['time_mean'],
            self.fraud_params['time_std']
        ))
        
        # Amount (frauds tend to be higher: $100-$2500)
        amount = np.random.uniform(100, 2500)
        
        # V features with higher variance and some suspicious patterns
        v_features = np.random.normal(
        ...
        # Make certain features more extreme
        suspicious_indices = np.random.choice(28, size=5, replace=False)
        for idx in suspicious_indices:
            v_features[idx] *= np.random.uniform(1.5, 3.0)
...
class SyntheticDataPipeline:
    ...
    def run_continuous(self):
        ...
        logger.info(f"⏱️  Will generate every {self.interval} seconds")
